# 대학원 연구와 별도 과제 확장 통합 설계서

> 보관 문서: 현재 대학원 연구 본체는 4-Agent 기반 Kubernetes 장애 감시/복구 실험이다. 이 문서는 별도 과제 요구를 같은 Agent 구조에 연결할 수 있는지 검토했던 중간 설계 기록이며, README 본문 기준의 핵심 범위는 아니다.

## 1. 문서 목적

이 문서는 현재 `aiops_research` 프로젝트가 두 가지 요구를 어떻게 구분하고, 어디에서 통합하는지 설명한다.

첫 번째 흐름은 대학원 연구인 4-Agent 기반 AIOps 장애 감시/복구 연구이다. 핵심은 Kubernetes 서비스 장애를 관측하고, HA/응용관리/인프라/비용 Agent가 각자의 관점에서 판단한 뒤, 검증된 복구 action만 실행하는 것이다.

두 번째 흐름은 별도 과제 요구사항이다. 핵심은 Go 언어 개발, 최소 2종 이상 LLM 또는 코딩 Agent 활용, AI App 배포/운용, CPU/GPU VM 기반 배치, 향후 AWS/Azure/GCP 및 API/Swagger/MCP 연동이다.
...
대학원 연구는 실험 중심이고, 별도 과제는 산출물 중심이다. 두 흐름을 연결하면 연구 결과뿐 아니라 요구사항 정의서, 기능/API 가이드, 설치 가이드, 시험 가이드까지 함께 제출할 수 있다.
...
> 본 연구는 Kubernetes 장애 복구를 위한 4-Agent AIOps 구조를 중심으로 시작했으며, 별도 과제 요구사항에 맞추어 Go 기반 안전 검증, LLM 교차 검증, CPU/GPU VM 기반 AI 응용 배포/운용 계획 기능을 확장할 수 있음을 확인하였다. 다만 현재 대학원 연구 본문에서는 장애 감시/복구 실험을 중심으로 다루고, AI App 배포/운용 기능은 보조 또는 후속 확장으로 분리한다.
